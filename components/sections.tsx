...
export function WhyUs() {
  return <section id="about" className="editorial-section why-section"><div className="shell editorial-split"><div className="editorial-intro"><h2>A focused partner for a website that has a <span className="hero-editorial-accent">real job to do.</span></h2><p>We combine sharp creative direction with the discipline of a sales page. The result feels premium, but never gets in the way of the message.</p><Link href="/about" className="text-link">Meet the studio <ArrowRight size={17}/></Link></div><ol className="value-list">{values.map(([title,copy],index)=><li key={title} data-reveal><span>{String(index+1).padStart(2,"0")}</span><div><h3>{title}</h3><p>{copy}</p></div></li>)}</ol></div></section>;
}
...
export function Process() {
  return <section id="process" className="editorial-section process-section"><div className="shell editorial-split"><div className="process-intro"><h2>A clear path from first conversation to <span className="hero-editorial-accent">launch.</span></h2><p>You always know what is happening, what we need from you, and what comes next.</p></div><ol className="process-list">{steps.map(([number,title,copy])=><li key={number} data-reveal><span>{number}</span><div><h3>{title}</h3><p>{copy}</p></div></li>)}</ol></div></section>;
}
...
export function Results() {
  return <section id="outcomes" className="editorial-section outcomes-section"><div className="shell"><div className="outcomes-header"><h2>Better design is valuable when it makes <span className="hero-editorial-accent">business easier.</span></h2><p>We do not promise a magic conversion number. We build the clarity, credibility, and momentum your marketing needs to perform at its best.</p></div><ul className="outcomes-list">{outcomes.map((outcome,index)=><li key={outcome} data-reveal><span>{String(index+1).padStart(2,"0")}</span>{outcome}</li>)}</ul></div></section>;
}
...
export function AgencyStandards() {
  return <section className="editorial-section relationship-section"><div className="shell relationship-layout"><div><h2>Sharp creative work without the <span className="hero-editorial-accent">agency theatre.</span></h2><p>Clear conversations, honest recommendations, and a build process that respects your time and your business.</p></div><ol>{deliveryStandards.map(([title,copy],index)=><li key={title}><span>{String(index+1).padStart(2,"0")}</span><div><h3>{title}</h3><p>{copy}</p></div></li>)}</ol></div></section>;
}

export function FinalCta() {
  return <section className="final-cta-section"><div className="final-cta-ambient" aria-hidden="true"><div className="final-cta-aurora" /></div><div className="shell final-cta"><div className="final-cta-copy"><h2>Ready for a website that feels <span className="hero-editorial-accent">unmistakably yours?</span></h2><p>Tell us what you need the website to achieve. We will review the project and recommend the clearest path to design, build, and launch it well.</p></div><div className="final-cta-actions"><Link href="/contact" className="button">Get My Free Website Quote <ArrowRight size={18}/></Link><a href={`tel:${site.phoneHref}`} className="button-secondary"><Phone size={17}/>{site.phoneDisplay}</a></div></div></section>;
}
